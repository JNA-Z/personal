...
// 发送一个字节
void w25qxx_send_byte(w25qxx_t* self, u8 byte)
{
  g_w25qxx_port->spi_transmit(self->hspi, &byte, 1, 1000);
}

// 接收一个字节
int8_t w25qxx_receive_byte(w25qxx_t* self)
{
    u8 ret = 0;
...
//    u8 ret = 0;
//    
//    // 直接调用HAL_SPI_TransmitReceive实现全双工（等价原有两个独立调用，兼容所有W25QXX型号）
//    // 全系列W25QXX均支持该单字节全双工时序，无需额外适配型号
//    if (HAL_SPI_TransmitReceive(self->hspi, &byte, &ret, 1, 1000) != HAL_OK)
//    {
//        return -1; // 失败返回-1，保持接口兼容
//    }
//    
//    return (int8_t)ret;
//}
...
//{
//    u8 ret = 0;
//    u8 dummy = 0xFF; // W25QXX全系列通用dummy字节，用于驱动时钟
//    
//    
//    // 步骤1：发送目标字节（指令/地址），同时利用SPI外设缓存，让W25QXX返回对应数据
//    g_w25qxx_port->spi_transmit(self->hspi, &byte, 1, 1000);
//    
//    // 步骤2：立即发送dummy字节（0xFF），同时接收W25QXX上一步返回的有效数据（关键！解决读ID异常）
//    // 用dummy字节驱动时钟，读取上一步收发的缓存数据，等价于全双工
//    g_w25qxx_port->spi_transmit(self->hspi, &dummy, 1, 1000);
//    g_w25qxx_port->spi_receive(self->hspi, &ret, 1, 1000);
//    
//    
...
u16 w25qxx_read_device_id(w25qxx_t* self)
{
    ...
    ID = w25qxx_receive_byte(self) << 8;
    ID |= w25qxx_receive_byte(self);
...
u8 w25qxx_read_status_register(w25qxx_t* self)
{
    ...
    status = w25qxx_receive_byte(self);
